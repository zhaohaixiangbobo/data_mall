...
function App() {
  ...
  const [filters, setFilters] = useState<Filters>({ units: [], domains: [] });
  ...
  useEffect(() => {
    const fetchBaseData = async () => {
      try {
        const [filtersData, rankingData, statsData] = await Promise.all([
          getFilters(),
          getRanking(),
          getStats(),
        ]);
  ...
  useEffect(() => {
    const fetchApps = async () => {
      setLoading(true);
      try {
        const appsData = await getApps(selectedUnit, selectedDomain, searchQuery);
        setApps(appsData);
  ...
  }, [selectedUnit, selectedDomain, searchQuery]);
  ...
  return (
    <div className="min-h-screen bg-[#eaf4fc] flex flex-col font-sans text-gray-900">
      {/* 顶部 Banner 区域 */}
      <header className="relative w-full h-48 bg-gradient-to-r from-blue-300 via-cyan-200 to-blue-200 overflow-hidden shadow-sm">
        {/* 装饰图案 */}
        ...
        <div className="w-11/12 max-w-[1600px] mx-auto h-full relative flex items-center justify-between">
      ...
      {/* 主体内容 */}
      <main className="flex-1 w-11/12 max-w-[1600px] mx-auto py-4 w-full flex flex-col gap-4">
        <FilterSection
          filters={filters}
              ...
              <>
                ...
                <div className="mt-4 flex items-center justify-end space-x-4">
                  ...
                  <button 
                    onClick={handleNextPage}
                    disabled={currentPage === totalPages}
                    className="px-4 py-2 bg-teal-400 hover:bg-teal-500 text-white text-sm rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
                  >
                    查看更多 &gt;
                  </button>
                </div>
      ...
      {/* 底部 Banner 区域 */}
      <div className="w-full bg-gradient-to-r from-teal-400 via-cyan-400 to-blue-400 mt-6">
        <div className="w-11/12 max-w-[1600px] mx-auto py-8 relative flex justify-between items-center">
           <div className="flex space-x-12">
             <div className="text-3xl font-bold text-yellow-300 italic tracking-wider drop-shadow-md">搭好技术舞台</div>
             <div className="text-3xl font-bold text-yellow-300 italic tracking-wider drop-shadow-md">共唱数字好戏</div>
           </div>
           <div className="text-3xl font-bold text-yellow-300 italic tracking-wider drop-shadow-md">全省可复制</div>
        </div>
      </div>

      {/* 页脚 */}
      <footer className="bg-white py-4">
        <div className="w-11/12 max-w-[1600px] mx-auto text-center text-sm text-gray-500">
          <p>主管单位：天津市烟草烟草专卖局（公司） 联系地址：天津市和平区长春道20号</p> 
        </div>
      </footer>
    </div>
  );
